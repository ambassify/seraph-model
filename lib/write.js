...
function cypherCommit(node, opts, callback) {
  ...
  if (opts.returnQuery) {
      callback(null, { query: query, params: params }, opts);
      return;
  }
  ...
  this.db.query(query, params, function(err, result) {
    ...
    else callback(null, reassembleFromGraph.call(self, graph, result[0]), opts);
  });
}

function afterCommit(object, opts, callback) {
  if (opts.returnQuery) {
...
module.exports = {
  ...
  save: function saveModel(object, excludeComps, callback) {
    ...
    if (typeof excludeComps == 'function') {
      ...
      options.excludeComps = false;
    } else if (_.isObject(excludeComps)) {
      options = excludeComps;
    } else {
      options.excludeComps = excludeComps;
    }

    return module.exports._save.call(this, object, options, callback);
  },
  ...
  saveComposition: function saveComposition(root, compName, objects, opts, callback) {
    ...
    async.waterfall([
      ...
      function runAfterCommit(shell, opts, cb) {
        if (opts.returnQuery) {
          callback(null, shell);
          return;
        }
